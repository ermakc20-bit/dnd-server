# -------- GM-ПАНЕЛЬ (С ОТОБРАЖЕНИЕМ СТОЛОВ) --------
...
async def gm_dashboard(request: Request, user_id: int):
    ...
    if tables:
        for table in tables:
            # Получаем сеттинг для таблицы
            setting_name = "Неизвестный сеттинг"
            for s in settings:
                if s.id == table.setting_id:
                    setting_name = s.name
                    break
            tables_html += f'''
            <div class="table-card">
                ...
                <div class="table-info">
                    ...
                    <div class="table-setting">{setting_name}</div>
                    <div class="table-link">/join/{table.link}</div>
                </div>
                <div class="table-actions">
                    <button onclick="copyLink('/join/{table.link}')" class="btn btn-small">📋</button>
                    <button onclick="deleteTable({table.id})" class="btn btn-small btn-danger">🗑️</button>
                </div>
            </div>
            '''
    else:
        tables_html = '<p style="color: #6b4c3b; text-align: center; padding: 20px;">У вас пока нет созданных столов. Нажмите "Создать игру" выше.</p>'
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .action-panel {{ padding: 20px; background: #16162a; border-radius: 12px; margin-bottom: 20px; display: flex; gap: 15px; flex-wrap: wrap; align-items: center; }}
        ...
        .table-icon {{ font-size: 30px; color: #c7a252; }}
        .table-info {{ flex: 1; }}
        .table-name {{ font-weight: bold; color: #c9a87c; }}
        .table-setting {{ font-size: 12px; color: #8a7a6a; }}
    ...
    <h3 style="color: #c9a87c; margin-bottom: 15px;">📋 Мои столы</h3>
